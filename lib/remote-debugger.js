...
class RemoteDebugger extends events.EventEmitter {
  ...
  constructor (opts = {}) {
    ...
    if (this.debuggerType === DEBUGGER_TYPES.webinspector) {
      ...
      this.pageLoadMs = pageLoadMs;                                 //fixme : kool maybe this
  ...
  async pageLoad (startPageLoadMs, pageLoadVerifyHook) {
    let timeoutMs = 3000;                                  // FixMe: kool: chasnged from 500 to 3000
    ...
    let verify = async () => {
      ...
      if (_.isFunction(pageLoadVerifyHook)) {         // FixMe: here see kool
        await pageLoadVerifyHook();
      }

      let ready = await this.checkPageIsReady();      // FixMe: or here
  ...
  async waitForFrameNavigated () {
    return new Promise(async (resolve, reject) => {
      ...
      if (!this.useNewSafari || this.pageLoadMs >= 0) {
        ...
        let timeout = this.useNewSafari ? this.pageLoadMs : 500;  /// fixme kool:  HERE !!!??
